...
class LoadingButton @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        val (cornerRadius, backgroundWidth, backgroundHeight) = setDefaultSettings()
    ...
    private fun setDefaultSettings(): Triple<Float, Float, Float> {
        val cornerRadius = 10.0f
        val backgroundWidth = measuredWidth.toFloat()
        val backgroundHeight = measuredHeight.toFloat()
        return Triple(cornerRadius, backgroundWidth, backgroundHeight)
    }
